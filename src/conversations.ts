...
export const thresholdSetupConversation: ConversationFn<MyContext> = async (
  ...
  try {
    ...
    console.error("Threshold conversation error:", (err as Error).message);
    try {
      await ctx.reply("Something went wrong. Please try again or use /help for options.", {
        reply_markup: mainMenuKeyboard(),
      });
    } catch {
...
export const quietHoursSetupConversation: ConversationFn<MyContext> = async (
  ...
  try {
    ...
    console.error("Quiet hours conversation error:", (err as Error).message);
    try {
      await ctx.reply("Something went wrong. Please try again or use /help for options.", {
        reply_markup: mainMenuKeyboard(),
      });
...
export const priceRequestConversation: ConversationFn<MyContext> = async (
  ...
  try {
    ...
    try {
      ...
      console.error("Price request error:", (err as Error).message);
      await ctx.reply(
        `Unable to fetch price data for ${coinText}. The service may be temporarily unavailable. Please try again in a moment.`,
        { reply_markup: backToMainKeyboard() },
      );
    }
  } catch (err) {
    console.error("Price request conversation error:", (err as Error).message);
    try {
      await ctx.reply("Something went wrong. Please try again or use /help for options.", {
        reply_markup: mainMenuKeyboard(),
      });
